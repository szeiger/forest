...
class APMBTreeBenchmark {
  ...
  var t1: APMBTree.Tree[Int, Int] = _
  var perm: Array[Int] = _ // repeatably pseudo-random permutation

  @Setup(Level.Trial) def init: Unit = {
    nums = 1 to size
    t1 = APMBTree.from(nums.iterator.map(x => (x, x+1)))
  ...
  def buildRandom(bh: Blackhole): Unit = {
    val t: APMBTree.Tree[Int, Int] = APMBTree.Tree.empty
    val i = nums.iterator
    while (i.hasNext) {
      val x = i.next()
      APMBTree.insert(t, x, x+1)
    }
    bh.consume(t)
  }
  ...
  def lookup(bh: Blackhole): Unit = {
    var i = 0
    while(i < perm.length) {
      bh.consume(APMBTree.get(t1, perm(i)))
      i += 1
    }
  }
}
