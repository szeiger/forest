...
class ThreeArrayBTreeBenchmark {
  ...
  var t1: ThreeArrayBTree.Tree[Int, Int] = _
  var perm: Array[Int] = _ // repeatably pseudo-random permutation

  @Setup(Level.Trial) def init: Unit = {
    nums = 1 to size
    t1 = ThreeArrayBTree.from(nums.iterator.map(x => (x, x+1)))
  ...
  def buildRandom(bh: Blackhole): Unit = {
    var t: ThreeArrayBTree.Tree[Int, Int] = ThreeArrayBTree.Tree.empty
    val i = nums.iterator
    while (i.hasNext) {
      val x = i.next()
      t = ThreeArrayBTree.insert(t, x, x+1)
    }
    bh.consume(t)
  }
  ...
  def foreach(bh: Blackhole): Unit = {
    ThreeArrayBTree.foreach(t1, ((kv: (Int, Int)) => bh.consume(kv)))
  }
  ...
  def lookup(bh: Blackhole): Unit = {
    var i = 0
    while(i < perm.length) {
      bh.consume(ThreeArrayBTree.get(t1, perm(i)))
      i += 1
    }
  }
}
